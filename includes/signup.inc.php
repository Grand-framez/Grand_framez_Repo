...
if (empty($fName) || empty($lName) || empty($email) || empty($password) || empty($passwordRepeat)) {
    header("location: ../signup.php?error=emptyfields&fName=".$fName."&lName=".$lName."&mail=".$email);
    exit();
}
else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("location: ../signup.php?error=invalidmail&fName=".$fName."&lName=".$lName);
    exit();
}
    ...
    if($result->num_rows==1){
        header("location: ../signup.php?error=userregistered");
        exit();
    }
