...
<p class="text-center mainmenu-area" style="margin-top:20px; font-size:30px; color:white;"> List of Photographers</p>
<?php
...
if($result->num_rows > 0){
    ...
    echo '<thead>';
    echo '<tr>';
    echo '<th scope="col">Photographer ID </th>';
    ...
    while($row = $result->fetch_assoc()){
        ...
        echo '<td>';
        echo '<form action="editemp.php" method="POST" style="display: inline-block; float: left;">';
        echo '<input type="hidden" name="id" value='.$row["empid"].'><button type ="submit" class="btn btn-info mr-3" name="edit" value="Edit" > <i class="fa fa-pencil"></i></button>';
        echo '</form>';

        echo '<form action="" method="POST" style="display: inline-block;  margin-left: 5px; float: left;">';
        echo '<input type="hidden" name="id" value='.$row["empid"].'><button type ="submit" class="btn btn-info mr-3" name="delete" value="Delete" > <i class="fa fa-trash"></i></button>';
        echo '</form>';
...
$sql = "DELETE FROM photographers WHERE empid = {$_REQUEST['id']}";
...
<div class="float-right" style="margin-left:670px;"><a href = "insertemp.php" style="color:#1abc9c;"><i class="fa fa-plus fa-2x"></i></a></div><br>
